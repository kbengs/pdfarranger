...
class _OffsetWidget(Gtk.Frame):
    ...
    def set_val(self, values, cursor_name):
        """Set left, right, top, bottom offset from dest page edges."""

        # Calculate the needed rescale to fill the rectangle
        if cursor_name != 'move':
            dpage, lpage = [page for [page] in self.damodel]
            dw, dh = dpage.size_in_pixel()
            lw, lh = lpage.size_in_pixel()
            w_rect = max(10, dw * (1 - values.left - values.right))
            h_rect = max(10, dh * (1 - values.top - values.bottom))
            lpage.scale *= min(w_rect / lw, h_rect / lh)
            self.rescale = lpage.scale / self.scale_old[0]
            for num, lp in enumerate(lpage.layerpages):
                lp.scale = self.rescale * self.scale_old[num + 1]
    ...
    def transform_scale(self):
        ...
        left/top 100%: pasted page is all to left/top
