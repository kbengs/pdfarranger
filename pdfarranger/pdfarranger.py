...
class PdfArranger(Gtk.Application):
    ...
    def iv_dnd_motion(self, iconview, context, x, y, etime):
        """Handles drag motion: autoscroll, select move or copy, select drag location feedback."""
        ...
        # In Windows when destination is another instance copy is always selected
        drag_move_posix = os.name == 'posix' and context.get_actions() & Gdk.DragAction.MOVE
        drag_move_nt = os.name == 'nt' and context.get_suggested_action() & Gdk.DragAction.MOVE
        ...
        if item:
            ...
            item_plus = iconview.get_dest_item_at_pos(x + cell_width / 2, y)
            ...
            if not path:  # drag location "cursor" not visible
